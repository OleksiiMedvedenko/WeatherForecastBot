...
namespace TelegramBot
{
    class Program
    {
        ...
        private static async void OnMessageHandler(object sender, MessageEventArgs e)
        {
            ...
            while(message.Text != "Повернутись до вибору міста")
            {
                switch (message.Text)
                {
                    case "Прогноз погоди на дану годину":
                        weatherResponse = OneDayWeatherController.GetWeatherFromWebSite("Wroclaw");
                        var Wroclaw = await client.SendTextMessageAsync(message.Chat.Id, "Прогноз погоди на дану годину",
        ...
        private static IReplyMarkup GetOneDayButtons()
        {
            return new ReplyKeyboardMarkup
            {
                Keyboard = new List<List<KeyboardButton>>
